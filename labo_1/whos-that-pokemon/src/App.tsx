...
function App() {
  ...
  let pokemonRandomNumber = randomPokemonNumber(pokemons.length);
  let rightPokemon: Pokemon = pokemons[pokemonRandomNumber - 1];
  console.log(rightPokemon);
  let pokemonOptions: Pokemon[] = [];
  do{
    let wrongPokemon = pokemons[pokemonRandomNumber];
    console.log(wrongPokemon);
    if(wrongPokemon !== rightPokemon){
      pokemonOptions.push(wrongPokemon);
    }
  }while(pokemonOptions.length !== 3)
  pokemonOptions.push(rightPokemon);
  ...
  return (
    <>
      <img src={"https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/"+pokemonRandomNumber+".png"} alt="" style={{filter: "brightness(0)"}}/>
      {pokemonOptions.map((pokemon: Pokemon) =>  <button type="button">{pokemon.name}</button> )}
    </>
  )
  function randomPokemonNumber(maxSize: number){
